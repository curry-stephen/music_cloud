<template>
  <div class='play-bar' :class='{ show: !toggle }'>
    <div class='fold' :class='{ turn: toggle }'>
      <young-icon :icon="iconList.ZHEDIE" @click='toggle=!toggle'></young-icon>
    </div>
    <!--播放进度-->
    ...
    <div class='control-box'>
      ...
      <div class="song-ctr">
        <young-icon :icon="playStateList[playStateIndex]" @click='changePlayState'></young-icon>
        <!--上一首-->
        <young-icon :icon="iconList.SHANGYISHOU" @click='prev'></young-icon>
        <!--播放-->
        <young-icon :icon="playBtnIcon" @click='togglePlay'></young-icon>
        <!--下一首-->
        <young-icon :icon="iconList.XIAYISHOU" @click='next'></young-icon>
        <!--音量-->
        <el-dropdown trigger="click">
          <young-icon v-if='volume !== 0' :icon="iconList.YINLIANG"></young-icon>
          <young-icon v-else :icon="iconList.JINGYIN"></young-icon>
          <template #dropdown>
            <el-dropdown-menu>
      ...
      <div class="song-ctr song-edit">
        <!--收藏-->
        <young-icon :class='{ active: isCollection }' :icon="iconList.XIHUAN" @click='collection'></young-icon>
        <!--下载-->
        <young-icon :icon="iconList.XIAZAI" @click='downloadMusic'></young-icon>
        <!--歌曲列表-->
        <young-icon :icon="iconList.LIEBIAO" @click='changeAside'></young-icon>
      </div>
    </div>
  </div>
</template>
...
import YoungIcon from './YoungIcon'
...
@import '@/assets/css/yy-play-bar.scss';
</style>
